    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or mpdiou or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or mpdiou or WIoU or ShapeIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                return iou - (rho2 / c2 + v * alpha),torch.pow(inter / (union + eps), gamma)  #Focal CIOU
            elif EIoU:
                ...
                return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2),torch.pow(inter / (union + eps), gamma)  #Focal EIoU
            elif SIoU:
                ...
                # return iou - 0.5 * (distance_cost + shape_cost) + eps,torch.pow(inter / (union + eps), gamma)  #Focal SIoU
            elif ShapeIoU:
                ...
                shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                # return iou - distance - 0.5 * shape_cost #ShapeIOU
                return iou - distance - 0.5 * shape_cost,torch.pow(inter / (union + eps), gamma) #FocalShapeIOU
            elif mpdiou:
                d1 = (b2_x1 - b1_x1) ** 2 + (b2_y1 - b1_y1) ** 2
                d2 = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
                return iou - d1 / hw.unsqueeze(1) - d2 / hw.unsqueeze(1)  # MPDIoU
                # return iou - d1 / hw.unsqueeze(1) - d2 / hw.unsqueeze(1),torch.pow(inter / (union + eps), gamma) # Focal MPDIoU
            elif WIoU:
                self = WIoU_Scale(1 - iou)
                dist = getattr(WIoU_Scale, '_scaled_loss')(self)
                return iou * dist  # WIoU https://arxiv.org/abs/2301.10051，WIOU不支持Focal
            return iou - rho2 / c2  # DIoU
            # return iou - rho2 / c2 ,torch.pow(inter / (union + eps), gamma) #Focal DIoU
        c_area = cw * ch + eps  # convex area
        return iou - (c_area - union) / c_area   # GIoU https://arxiv.org/pdf/1902.09630.pdf
        # return iou - (c_area - union) / c_area，torch.pow(inter / (union + eps), gamma)  #Focal GIoU
    # return iou  # IoU
