...
class BaseTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.distill_feat_type = "mimic"  # "cwd","mgd","mimic"  #cwd基于特征蒸馏
        self.distillonline = True  # False or True
        self.logit_loss = True # False or True   #是否启用逻辑损失，比较教师模型和学生模型的输出分布
        self.distill_layers = [2, 4, 6, 8, 12, 15, 18, 21]   #在这几层提取特征图进行蒸馏
        # ------------------------------Add-Param-End-----------------
    ...
    def _setup_train(self, world_size):
        ...
        if self.model_t is not None:     #将教师模型的参数设置为可训练模式
        ...
        if world_size > 1:
            ...
            if self.model_t is not None:
                self.model_t = nn.parallel.DistributedDataParallel(self.model_t, device_ids=[RANK],find_unused_parameters=True)
    ...
    def _do_train(self, world_size=1):
        ...
        if self.model_t is not None:
            ...
            self.distillloss = FeatureLoss(channels_s=self.channels_s, channels_t=self.channels_t, distiller= self.distill_feat_type)  #蒸馏
        if world_size > 1:
                ...
                if RANK in {-1, 0}:
                    ...
                    pbar.set_description(
                        ...
                        (f'{epoch + 1}/{self.epochs}',
                         f"{self._get_memory():.3g}G",  # (GB) GPU memory util
                         *(self.tloss if loss_length > 1 else torch.unsqueeze(self.tloss, 0)),  # losses
                         self.featureloss, self.teacherloss, self.logitloss, batch['cls'].shape[0], batch['img'].shape[-1]))
